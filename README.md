...
- **Phase 4.5 (2026-04-22).** `ehc_contracts.metrics.lb_margin` — canonical column constants `FLOW_COLS` (9-col, Rule 1) and `LB_MARGIN_COLS` (7-col, Jeff's A1-ruling for origination_detail) plus `lb_margin(rows, cols=FLOW_COLS)` function. Scope: Formulas 1 and 2 (column-sum primitives). Three hybrid formulas in emitters (Dashboard Investment Summary, Dashboard Reforecast Margin, Reporting App fund-to-date per-project) are intentionally distinct metrics and preserved in-place. No R twin — R-side uses Formula 5 which is its own metric. Closes BACKLOG 4.1.1 step 1 (R-side `LB_Margin = "P"` override hoisted globally).
